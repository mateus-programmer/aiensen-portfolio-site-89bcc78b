...
import { Plus, Pencil, Trash2, LogOut, Home, Eye, EyeOff } from "lucide-react";
...
const AdminPanel = () => {
  ...
  const fetchCategories = async () => {
    // Admin needs to see all categories including inactive
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) toast.error("Erro ao excluir"); else { toast.success("Excluído!"); fetchCategories(); }
  };

  const handleToggleActive = async (cat: Category) => {
    const { error } = await supabase.from("categories").update({ is_active: !cat.is_active }).eq("id", cat.id);
    if (error) toast.error("Erro"); else fetchCategories();
  };
