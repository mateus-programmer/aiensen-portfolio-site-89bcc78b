...
import { topics } from "@/components/TheologyTopicCards";
...
const TheologySubPage = () => {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ["content_items", id, "theology", slug, user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq("category_id", id!)
        .eq("is_active", true)
        .contains("tags", [topic?.name || ""])
        .order("sort_order", { ascending: true });
      if (error) throw error;
      return data as ContentItem[];
    },
    enabled: !!id && !!user && !!topic,
  });

  if (!topic) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        <p className="text-muted-foreground font-alt">Tópico não encontrado.</p>
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center gap-4">
          ...
          <h1 className="font-display text-lg font-bold" style={{ color: `hsl(${topic.accentHsl})` }}>
            {topic.name}
          </h1>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        {!user && !authLoading ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
            ...
            <button
              onClick={() => navigate(`/auth?redirect=${encodeURIComponent(`/categoria/${id}/topico/${slug}`)}`)}
          ...
          <>
            {isAdmin && id && (
              <PdfUpload
                categoryId={id}
                defaultTags={[topic.name]}
                onUploadComplete={() => queryClient.invalidateQueries({ queryKey: ["content_items", id, "theology", slug] })}
              />
            )}
            ...
            {items && items.length > 0 && (
              <div className="relative max-w-md mb-6">
                ...
                <input
                  ...
                  placeholder={`Buscar em ${topic.name}...`}
            ...
            {(() => {
                      ...
                      if (item.file_url) {
                        return (
                          <PdfFileItem
                            ...
                            index={i}
                            isAdmin={isAdmin}
                            onDeleted={() => queryClient.invalidateQueries({ queryKey: ["content_items", id, "theology", slug] })}
                          />
                        );
                      }
                      return (
                        <motion.div
                          ...
                          className="bg-card border border-border rounded-xl p-5 border-l-4 border-l-neon-purple hover:bg-secondary/30 transition-colors"
                        >
              ...
              return (
                <div className="text-center py-20">
                  ...
                  <p className="font-alt text-muted-foreground text-sm">Nenhum conteúdo disponível para {topic.name} ainda.</p>
                </div>
              );
            })()}
          </>
...
export default TheologySubPage;
